...
import { TokenAmount } from '../../../local_modules/swap-sdk'
...
const StakeModal = ({ isOpen, onDismiss, yearStake, stakeAmount }: StakeModalProps) => {
  ...
  const currency = new TokenAmount(CRO_TOKEN, isOpen ? parseUnits(stakeAmount || '0', 18).toString() : '0')
    ...
    try {
      ...
      if (shouldCall) {
        const result: TransactionResponse = await contract?.stake(
          parseUnits(stakeAmount || '0', 18).toString(),
          Buffer.from('0x')
        )
